...
function SelectBox({
  ...
  function HeaderComponent() {
    ...
    const { SelectUnSelectAll, options } = props;
    return (
      <>
      {isMulti ? (
      <View style={{flexDirection:'row',width:"100%",justifyContent:'space-around',marginTop:3,position:'relative'}}>
      <TouchableOpacity style={button} onPress={() => {onPressSelectUnSelectAll(true)}}><Text >Select All</Text></TouchableOpacity>
      <TouchableOpacity style={button} onPress={() => {onPressSelectUnSelectAll(false)}}><Text >Un-Select All</Text></TouchableOpacity>
      </View>
      ) : null}
        {!hideInputFilter && (
    ...
    function onPressSelectUnSelectAll(selected) {
      let value=[];
      if(selected){
        value = options;
      }
      SelectUnSelectAll ? SelectUnSelectAll(value) : null;
    }
  }
